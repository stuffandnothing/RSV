...
On Void (and any distro whose default runsvdir is `/var/service`), that same check also covers chroots: `/var/service` only reflects reality once runit is actually supervising it, so whenever runit isn't running, rsv operates on `/etc/runit/runsvdir/default` instead — matching Void's own documented chroot install steps (and how Void's installer itself enables services pre-boot).
